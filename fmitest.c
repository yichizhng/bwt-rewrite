...
void rna_seq(const fm_index *fmi, const unsigned char *pattern, long long len) {
  // Copy pattern into a different aray for cyclic search
  unsigned char *pat = malloc(2*len);
...
void startWorkers(fm_index *fmi, long long num, unsigned char *pats) {
